...
class TestIndexView(TestCase):
    ...
    def test_view_in_normal_conditions(self):
        ...
        query_recipes = response.context.get("recipe_list_pagination")
    ...
    def test_view_with_parameters(self):
        ...
        self.assertQuerySetEqual(
            Recipe.objects.filter(tags__name="Beef")[:12],
            response.context.get("recipe_list_pagination"),
        )
...
class TestRecipeDetailView(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.recipe = Recipe.objects.create(name="French Fries")
...
class TestRecipeListView(TestCase):
    ...
    def test_with_parameters(self):
        response = self.client.get(
            f"{reverse('recipe-list')}?{urlencode({'query': 'dish'})}"
        )
        ...
        self.assertListEqual(
            response.context.get("recipe_list_pagination").object_list, [self.main_dish, self.dish]
        )

        response = self.client.get(
            f"{reverse('recipe-list')}?{urlencode({'query': ''})}"
        )
        self.assertQuerySetEqual(
            Recipe.objects.all()[:12], response.context.get("recipe_list_pagination")
        )
